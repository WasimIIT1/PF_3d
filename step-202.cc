/*Shear test with splitting: Phasefield*/
...
private:
	...
	SymmetricTensor<2,2> strain;//strain tensor(second order, 2 dimensions)
...
*/
	//CALCULATING HPLUS as per thesis:
	float tr_strain,Mac_tr_strain,Mac_Max_principal_strain,Mac_Min_principal_strain,tr_sqr_Mac_Principal_strain;
	tr_strain = e_xx + e_yy;
	if(tr_strain >0)
	{
...
// eigenvalues(strain):eigenvalues of a symmetric 2×2 tensor stored in descending order
    std::array<double, 2 > Principal_strains = eigenvalues(strain);
    if (Principal_strains[0]>0)
    {
    	Mac_Max_principal_strain = Principal_strains[0];
    }
    else
    {
    	Mac_Max_principal_strain = 0;
    }
    if (Principal_strains[1]>0)
        {
        	Mac_Min_principal_strain = Principal_strains[1];
        }
        else
        {
        	Mac_Min_principal_strain = 0;
        }
    tr_sqr_Mac_Principal_strain = pow(Mac_Max_principal_strain,2)+pow(Mac_Min_principal_strain,2);
	...
	// Calculating Hplus as per MATLAB CODE:
...

{
	...
	MatrixTools::apply_boundary_values(boundary_values_elastic,
			...
			solution_elastic,
			system_rhs_elastic/*, eliminate_columns = false*/);
