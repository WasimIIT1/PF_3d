/*Shear test with splitting (3D): Phasefield*/
	...
	for (const auto &cell : dof_handler_elastic.active_cell_iterators())
	{
		for (const auto &face : cell->face_iterators())
		{
			if (face->at_boundary())
			{
				...
				if (std::fabs(center(1) - (0)) < 1e-12) //face lies at y=0
				{

					for (const auto vertex_number : cell->vertex_indices())
					{
						const auto vert = cell->vertex(vertex_number);
						if (std::fabs(vert(1) - 0) < 1e-12 ) //vertex lies at y=0;
						{
				...
				if (std::fabs(center(2) - (0)) < 1e-12 ||
						std::fabs(center(2) - (0.005)) < 1e-12) //face lies at z=0 or z=0.005
				{

					for (const auto vertex_number : cell->vertex_indices())
					{
						const auto vert = cell->vertex(vertex_number);
						if (std::fabs(vert(2) - 0) < 1e-12 ||
								std::fabs(center(2) - (0.005)) < 1e-12) //vertex lies at y=0;
						{
...

{
	...
	for (const auto &cell : dof_handler_elastic.active_cell_iterators())
	{
	 ...
	 std::cout << "system_matrix_elastic(36,36)" << system_matrix_elastic.operator()(36,36)<< std::endl;
	...
	MatrixTools::apply_boundary_values(boundary_values_elastic,
			...
			system_rhs_elastic);
	 std::cout << "system_matrix_elastic(36,36)" << system_matrix_elastic.operator()(36,36)<< std::endl;
	 std::cout << "system_matrix_elastic_copy(36,36)" << copy_of_system_matrix_elastic.operator()(36,36)<< std::endl;
...
for (int t=1;t<=t_max;t++)
{
	...
	//load displacement curves:
		const IndexSet dofs_applied_disp = DoFTools::extract_boundary_dofs(dof_handler_elastic,	 u_x_mask , {1});//1 is the boundary id
