/*Twisting beam with splitting (3D): Phasefield*/
// This program evaluates load displacement by running element loop
...
private:
	...
	Vector<double> load_disp(/*const FEValues<3> & fe_values_elastic,*/Vector<double> & solution_elastic);
	...
	float ux=5e-4; // increment in loading

	float l = 2.00; // length scale parametre
	int t_max = 10; // number of load steps
	float tol =1e-2; // tolerance forn error in solution
	float gc = 1.6e-4;//energy release rate
	int nx = 20;
	int ny = 4;
	int nz = 4;
...
void PhaseField::setup_system_elastic()
{
	...
	copy_of_system_matrix_elastic.reinit(sparsity_pattern_elastic);
...
Vector<double> PhaseField::load_disp(/*const FEValues<3> & fe_values_elastic,*/Vector<double> & solution_elastic)

{
	Timer timer_load_disp; // creating a timer also starts it
	...
	Vector<double>     element_solution(dofs_per_cell);

	Vector<double> force(dof_handler_elastic.n_dofs());
	...
	for (const auto &cell : dof_handler_elastic.active_cell_iterators())
	{
		...
		for (const auto vertex : cell->vertex_indices())
			{
				int a = (cell->vertex_dof_index(vertex, 0));
				element_solution[2*i]=  solution_elastic[a*2];
				element_solution[2*i+1]=  solution_elastic[(a*2)+1];
		...
		for (const unsigned int i : fe_values_elastic.dof_indices())
		{
			force(local_dof_indices[i]) += cell_force(i);
		}
	...
	timer_load_disp.stop();

	std::cout << "Elapsed wall time assemble_system_elastic: " << timer_load_disp.wall_time() << " seconds.\n";


	// reset timer for the next thing it shall do
	timer_load_disp.reset();
	return force;
		...
		for (const auto vertex_number : cell->vertex_indices())
					{
						...
						if (((std::fabs((z_max*(vert(0)-0.5*x_max + A)) - 2*A*( vert(2))) <10*l)
								&& vert(1)>=0.9*y_max)
								|| ((std::fabs((vert(0)-0.5*x_max + A)*z_max+2*A*(vert(2)-z_max))<10*l)
										&& vert(1)<=0.1*y_max))
						{

							// nodes on initial damage lines
...
//triangulation.refine_global(refinement_value);
...
for (int t=1;t<=t_max;t++)
{
	...
	force_vector =load_disp(/*fe_values_elastic,*/solution_elastic);
	//load displacement curves:
		const IndexSet dofs_applied_disp = DoFTools::extract_boundary_dofs(dof_handler_elastic,	 u_x_mask , {2});//4 is the boundary id
		...
		load_values[t] = load;
		displacement_values[t] = ux*t;



		 std::ofstream m_file;
		  m_file.open("load_displacement.m");
		  // m_file is saved in the same directory as .cc file
		  m_file << "% Matlab code generated by dealii to plot load displacement curves" << std::endl;
		  m_file << "clc" << std::endl;
		  m_file << "clear" << std::endl;
		  m_file << "close all" << std::endl;
		  m_file << "load=[" <<  load_values << "]"<< std::endl;
		  m_file << "displacement=[" << displacement_values <<"]" << std::endl;
		  m_file << "plot( displacement,load,'linewidth',1.8)" << std::endl;
		  m_file << "xlabel(\"Displacement (mm)\",'fontsize', 16)" << std::endl;
		  m_file << "ylabel(\"Load (kN)\",'fontsize', 16)" << std::endl;
		  m_file << "title(\"Load displacement curve- Twisting crack \",'fontsize', 16)" << std::endl;

		  m_file << "hold on" << std::endl;
		  m_file << "Table=readtable(\"plot-data.csv\")"<< std::endl;//needed if we want to compare with some existing plots
		  m_file <<"plot(Table.x,Table.y,'linewidth',1.8)"<< std::endl;

		  m_file << "set(gca,'fontname', 'Courier','FontSize',15,'FontWeight','bold'); grid on"<< std::endl;
		  m_file << "legend(\"deal.ii results\",\"Alba's paper\")"<< std::endl;
