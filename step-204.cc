...
// This program evaluates load displacement by storing global K
...
private:
	...
	const double x_max=1;const double y_max=1;const double z_max=0.005;
	float del_u=1e-4; // increment in loading

	float l = 0.015; // length scale parametre
	int t_max = 200; // number of load steps
	float tol =1e-2; // tolerance forn error in solution
	float gc = 2.7e-3;//energy release rate
	int nx = 256;
	int ny = 256;
	int nz = 1;
	...
	for (const auto &cell : dof_handler_elastic.active_cell_iterators())
	{
		for (const auto &face : cell->face_iterators())
		{
			if (face->at_boundary())
			{
				...
				if (std::fabs(center(1) - (y_min)) < 1e-12) //face lies at y=0
				{

					for (const auto vertex_number : cell->vertex_indices())
					{
						const auto vert = cell->vertex(vertex_number);
						if (std::fabs(vert(1) - y_min) < 1e-12 ) //vertex lies at y=0;
						{
				...
				if (std::fabs(center(2) - (z_min)) < 1e-12 ||
						std::fabs(center(2) - (z_max)) < 1e-12) //face lies at z=0 or z=0.005
				{

					for (const auto vertex_number : cell->vertex_indices())
					{
						const auto vert = cell->vertex(vertex_number);
						if (std::fabs(vert(2) - z_min) < 1e-12 ||
								std::fabs(center(2) - z_max) < 1e-12) //vertex lies at y=0;
						{
...
for (const auto &cell : triangulation.active_cell_iterators())
{
	for (const auto &face : cell->face_iterators())
	{
		...
		{
			const auto center = face->center();
			if (std::fabs(center(1) - (y_min)) < 1e-12) //left face
			{

				face->set_boundary_id(2);

			}
			else if (std::fabs(center(1) - y_max) < 1e-12) //right face
			{
	...
	//load displacement curves:
		...
		displacement_values[t] = del_u*t;
		 ...
		 std::ofstream m_file;
		  ...
		  m_file << "Table=readtable(\"alba_2d_shear_load_disp.csv\")"<< std::endl;//needed if we want to compare with some existing plots
