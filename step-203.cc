...
private:
	...
	const double x_min=0;const double y_min=0;
	const double x_max=1;const double y_max=1;
	float ux=1e-4; // increment in loading
	double pi = M_PI;

	float l = 0.015; // length scale parametre
	int t_max = 70; // number of load steps
	float tol =1e-2; // tolerance forn error in solution
	float gc = 2.7e-5;//energy release rate
	int refinement_value = 8;
	...
	for (const auto &cell : dof_handler_elastic.active_cell_iterators())
	{
		for (const auto &face : cell->face_iterators())
		{
			if (face->at_boundary())
			{
				...
				if (std::fabs(center(1) - (y_min)) < 1e-12) //face lies at bottom edge
				{
...

					{
						...
						if (std::fabs(vert(1) - (y_min)) < 1e-12)
...
void PhaseField::setup_boundary_values_damage()
{
	...
	for (const auto &cell : dof_handler_damage.active_cell_iterators())
	{
		...
		for (const auto vertex_number : cell->vertex_indices())
					{
						const auto vert = cell->vertex(vertex_number);
						if ((vert(0) - 0.5*(x_max+x_min) < 1e-12) &&
								(std::fabs(vert(1) -0.5*(y_max+y_min)) < 1e-12)) // nodes on initial damage line
						{   types::global_dof_index damage =
								cell->vertex_dof_index(vertex_number, 0);
...
for (const auto &cell : dof_handler_damage.active_cell_iterators())
{
	for (const auto vertex_number : cell->vertex_indices())
	{
		...
		if ( ((vert(0) - 0.5*(x_max+x_min)) < 1e-12) && (std::fabs(vert(1) - 0.5*(y_max+y_min)) < 1e-12))
		{
			solution_damage[a] = 1;
		}
		else
...
for (int t=1;t<=t_max;t++)
{
	...
	const IndexSet dofs_applied_disp = DoFTools::extract_boundary_dofs(dof_handler_elastic,	 u_x_mask , {4});//4 is the boundary id

	float load = 0;

	force_vector.reinit(dof_handler_elastic.n_dofs());
	std::cout << force_vector[1] << std::endl;
...
// Create a DataOut object and add the vectors to it
 /* DataOut<1,Vector<double> > data_out_load_disp;
  data_out_load_disp.add_data_vector (load_values, "load");
  data_out_load_disp.add_data_vector (displacement_values, "displacement");

  // Write the Gnuplot file
  data_out_load_disp.build_patches ();
  data_out_load_disp.write_gnuplot (deallog.get_file_stream());*/

/*
DataOut<1> data_outw;
  data_outw.add_data_vector(load_values, "a");
  data_outw.add_data_vector(displacement_values, "b");
  data_outw.build_patches();

  // Output the data to a file in VTK format

  std::ofstream output("plot.vtk");
  data_outw.write_vtk(output);
  output.close();
*/
