...
private:
	...
	float u_z=5e-4; // increment in loading
	double pi = M_PI;

	float l = 2.00; // length scale parametre
	float notch_depth = 0.1*l;
	int t_max = 500; // number of load steps
	float tol =1e-2; // tolerance forn error in solution
	float gc = 16e-6;//energy release rate

	int nx=256;int ny=40;int nz=40;
...
void PhaseField::setup_boundary_values_elastic(int t)
{
	...
	for (const auto &cell : dof_handler_elastic.active_cell_iterators())
	{
		for (const auto &face : cell->face_iterators())
		{
			...
			/*	else if (std::fabs(center(2) - (z_max)) < 1e-12) //face lies at z=z_max
				{
...
/*
	...
	VectorTools::interpolate_boundary_values(dof_handler_elastic,
					2,
					Functions::ConstantFunction<3>(u_z*t,3),
					boundary_values_elastic,u_x_mask_right
			);
...
void PhaseField::setup_boundary_values_damage()
{
	...
	for (const auto &cell : dof_handler_damage.active_cell_iterators())
	{
		...
		for (const auto vertex_number : cell->vertex_indices())
					{
						const auto vert = cell->vertex(vertex_number);
						if ((std::fabs(vert(1)-vert(0)-0.5*y_max+0.5*x_max)<l) && (vert(2)<=notch_depth))
						{
...
// Define the dimensions of the mesh
  ...
  const double x_max = 125.0;
  const double y_min = 0.0;
  const double y_max = 25.0;
  const double z_min = 0.0;
  const double z_max = 25.0;*/

  // Define the number of cells in each direction
  const unsigned int n_x_1 = 20; // 20 elements before x=42.5
  const unsigned int n_x_2 = 60; // 60 elements from x=42.5 to x=82.5
  const unsigned int n_x_3 = 20; // 20 elements after x=82.5
  const unsigned int n_y = 40;
  const unsigned int n_z = 40;

  // Compute the cell sizes in each direction
  const double h_x_1 = (42.5 - x_min) / n_x_1;
  const double h_x_2 = (82.5 - 42.5) / n_x_2;
  const double h_x_3 = (x_max - 82.5) / n_x_3;
  const double h_y = (y_max - y_min) / n_y;
  const double h_z = (z_max - z_min) / n_z;

  // Create a Triangulation object and a GridGenerator object
  GridGenerator::hyper_rectangle(triangulation, Point<3>({x_min, y_min, z_min}), Point<3>({42.5, y_max, z_max}));
  GridGenerator::subdivided_hyper_rectangle(triangulation, {n_x_1, n_y, n_z}, Point<3>({42.5, y_min, z_min}), Point<3>({42.5 + h_x_2 * n_x_2, y_max, z_max}), true);
  GridGenerator::subdivided_hyper_rectangle(triangulation, {n_x_3, n_y, n_z}, Point<3>({82.5, y_min, z_min}), Point<3>({x_max, y_max, z_max}), true);
  ...
  // Output the mesh to a file in VTK format
  GridOut grid_out;
...
for (const auto &cell : triangulation.active_cell_iterators())
{
	for (const auto &face : cell->face_iterators())
	{
		if (face->at_boundary())
		{
			...
			if (std::fabs(center(0) - x_max) < 1e-12) //right face
			{
