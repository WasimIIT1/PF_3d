// beam with initial notch along the plane x=0.5*x_max
// depth of notch in z=0.1*z_max
//We will solve a 3d problem that cannot be reduced to 2d//
//Mesh:256*40*40
//l=2; damage plane bandwidth=10l, on each side;
...
private:
	...
	float notch_depth = 0.1*z_max;
	int t_max = 200; // number of load steps
	float tol =1e-2; // tolerance forn error in solution
	float gc = 16e-5;//energy release rate

	int nx=200;int ny=40;int nz=40;

	float l = 2;
	float bandwidth = 0.5*l;
				...
				else if (std::fabs(center(2) - (z_max)) < 1e-12) //face lies at x=x_max
				{
					...
					for (const auto vertex_number : cell->vertex_indices())
					{
							...
							&&(std::fabs(vert(2) - z_max) < 1e-12 )) //vertex lies at 0.5*x_max,z_max;
						{
							...
							boundary_values_elastic[z_displacement] = del_u*t; //fixed in z
...
/*
	...
	double u_x_values_right = del_u*t;
...
void PhaseField::setup_boundary_values_damage()
{
	...
	for (const auto &cell : dof_handler_damage.active_cell_iterators())
	{
		...
		for (const auto vertex_number : cell->vertex_indices())
					{
						const auto vert = cell->vertex(vertex_number);
						if ((std::fabs(vert(0)-0.5*x_max)<bandwidth) && (vert(2)<=notch_depth))
						{
...
std::cout << "Initial Crack in yz plane:" << std::endl;
...
std::cout << "Number of degrees of freedom : " << dof_handler_elastic.n_dofs() + dof_handler_damage.n_dofs()
	   	                				   << std::endl;
...
std::cout << "Notch depth=" << notch_depth << std::endl;

std::cout << "Load increment   :"
		<< del_u << std::endl;
std::cout << "  Number of increments :"
		<< t_max << std::endl;
std::cout << "  tolerance  :"
		<< tol << std::endl;
